...
//#define DLMS_CRC_POLYNOM         0x8408


uint16_t DLMSCrcCalc(uint8_t * pAdd, uint16_t count);
...
//
//  ARGUMENTS: 
//      uint8_t_  crcData  - Data to perform the CRC-16 operation on.
...
uint16_t DLMSCrcCalc(uint8_t * pAdd, uint16_t count)
{
 ...
 //   unsigned char ch;
    ...
    {
       fcs16 = (fcs16 >> 8) ^ FCS16Table[(fcs16 ^ pAdd[pos]) & 0xFF];
    }
    fcs16 = ~fcs16;
